...
{
  openlog("server1", LOG_PID, LOG_USER);
  ...
  {
    std::cout << "[SERVER_1][ERROR][THREAD][INIT] " << strerror(errno) << "\n";
    return -1;
  }
  ...
  {
    std::cout << "[SERVER_1][ERROR][THREAD][STACK] " << strerror(errno) << "\n";
    return -1;
  }

  if (pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED) != 0)
  {
    std::cout << "[SERVER_1][ERROR][THREAD][DETACH] " << strerror(errno) << "\n";
    return -1;
  }
  // ???
  ...
  {
    std::cout << "[SERVER_1][ERROR] Can't create socket\n";
    return -1;
  }
  else
  {
    syslog(LOG_INFO, "[SERVER_1][ACCESS] Socket created log");
    std::cout << "[SERVER_1][ACCESS] Socket created\n";
  }
  ...
  {
    std::cout << "[SERVER_1][ERROR] " << strerror(errno) << "\n";
    return -1;
  }
  ...
  {
    syslog(LOG_INFO, "[SERVER_1][INFO] Waiting for incoming connections...");
    std::cout << "[SERVER_1][INFO] Waiting for incoming connections...\n";
  }
  ...
  while (true)
  {
    ...
    if (conn_id == -1)
      std::cout << "[SERVER_1][WARNING] Can't accept connection\n";
    else
    {
      // if connection limit reached
      if (connection >= CONCURRENT_CONNECTION)
      {
        std::cout << "[SERVER_1][WARNING] Connection Limit\n";
        close(conn_id); // close connection
      }
      else
      {
        syslog(LOG_INFO, "[SERVER_1][ACCESS] New connection accepted from %s:%d", inet_ntoa(client.sin_addr), ntohs(client.sin_port));
        std::cout << "[SERVER_1][ACCESS] New connection accepted from " << inet_ntoa(client.sin_addr) << ":" << ntohs(client.sin_port) << "\n";
        if (pthread_create(&thread_id, &attr, connection_handler, new int(conn_id)) == -1) // create new thread for new connection
        {
          std::cout << "[SERVER_1][WARNING] Can't create new thread\n";
          close(conn_id);
        }
        else
...
{
  // Print Thread ID
  syslog(LOG_INFO, "[SERVER_1][INFO] THREAD ID = %ld", pthread_self());
  printf("[SERVER_1][INFO] THREAD ID = %ld\n", pthread_self());
  ...
  {
    syslog(LOG_INFO, "[SERVER_1][RECEIVED] %s", buffer);
    std::cout << "[SERVER_1][RECEIVED] " << buffer << "\n";
    ...
    else if (strcmp(buffer, "2") == 0)
    {
      ...
      if (strcmp(fileBuffer, " "))
        sprintf(response, "%s", "No Display");
      else
        sprintf(response, "%s", fileBuffer);
    }
    else
    ...
    {
      syslog(LOG_INFO, "[SERVER_1][SEND] %s", response);
      std::cout << "[SERVER_1][SEND] " << response << "\n";
    }

    else
      std::cout << "[SERVER_1][WARNING] " << strerror(errno) << "\n";
  }

  sleep(1);       // Sleep
  close(conn_id); // terminate connection
  syslog(LOG_INFO, "[SERVER_1][INFO] CONNECTION CLOSED");
  std::cout << "[SERVER_1][INFO] CONNECTION CLOSED\n"; // Close Connection
